...
from header_skills import skl_ironflesh
...
extra_damage_by_power_skill =(
  ti_on_agent_hit, 0, 0, [],
    [
        (store_trigger_param_1, ":inflicted_agent"),#inflicted_agent
        (store_trigger_param_2, ":dealer_agent"),#dealer_agent
        (store_trigger_param_3, ":inflicted_damage"),#inflicted_damage
        (assign, reg10, ":inflicted_damage"),
        (assign, reg15, reg10),
        (assign, ":dealer_item_id", reg0),#weapon used
        (try_begin),
            (gt, ":dealer_item_id", -1),
            (str_store_item_name, s5, ":dealer_item_id"),
            (str_store_agent_name, s2, ":inflicted_agent"),
            (str_store_agent_name, s4, ":dealer_agent"),
#             (display_message, "@Weapon: {s5}, weapon id:{reg0},  attacker:{s4}"),
    #if is enemy, deliver extra damage
            (neg|agent_is_ally, ":inflicted_agent"),
            (agent_get_troop_id, ":dealer_trp_id", ":dealer_agent"),
            (str_store_string, s9, ":dealer_trp_id"),
            (store_agent_hit_points,reg11,":inflicted_agent",1),
            (assign, reg12, reg11),
            (try_begin),
                ...
                (try_begin),
                    ...
                    (store_skill_level, reg9,"skl_power_draw",":dealer_trp_id"),
                    (str_store_string, s6, "str_power_draw"),
                (else_try_begin),
                    #for thrown weapon
                    (eq, itp_type_thrown, ":weapon_type"),
                    (store_skill_level, reg9,"skl_power_throw",":dealer_trp_id"),
                    (str_store_string, s6, "str_power_throw"),
                (else_try),
                    #for melee weapon
                    (store_skill_level, reg9,"skl_power_strike",":dealer_trp_id"),
                    (str_store_string, s6, "str_power_strike"),
                (try_end),

                (assign, reg14,reg9),
#                 (val_mul, reg9, 2),
                (store_random_in_range, reg13, 0, reg14),#generate extra damage between the POWER STRIKE skill level and twice value of the skill level
                (val_add, reg10, reg13),
                (try_begin),
                    (gt, reg13, 0),
                    (agent_is_human, ":inflicted_agent"),
                    (display_message, "str_extra_power_strike_damage",0xFF00FF),
                (try_end),
                (try_begin),
                    (neg|gt, reg10, reg12),
                    (val_sub, reg11, reg10),
                    (agent_set_hit_points,":inflicted_agent",reg11,1),
                (else_try),
                    (agent_set_hit_points,":inflicted_agent",0,1),
                (try_end),
                (assign, ":inflicted_damage", reg10),
...
recover_life_from_item =(
  10, 0, 0, [],#每10秒恢复一次
    [
        (call_script, "script_recover_life_from_item", "itm_lqq", 1),#装备龙骑银枪，恢复1点
    ]
)
