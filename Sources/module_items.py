...
# (display_message, "@{!}DEBUG --  弓 ， 弩 ， 投 掷 ， 枪 ，  连 射 模 式 触 发 器 开 始  "),
    (store_trigger_param_1,":attacker"),
(try_begin),
    (agent_get_troop_id,":attacker_trp_id",":attacker"),
    (agent_get_wielded_item,":attacker_weapon",":attacker"),
    (gt,":attacker_weapon",0),
#     (display_message, "@{!}DEBUG --  攻 击 武 器 是 正 常 武 器 "),
    (item_get_type,":attacker_weapon_type",":attacker_weapon"),
    (is_between,":attacker_weapon_type",8,18),
#     (display_message, "@{!}DEBUG --  攻 击 武 器 类 型  {:attacker_weapon_type_str} 在   8 - - 1 8   之 间 , 符 合 条 件 "),
    (get_player_agent_no,":agent_no"),
    (assign,":var_5",0),
    (assign,":var_6",0),
    (try_begin),
        (neg|eq,":agent_no",":attacker"),
#         (display_message, "@{!}DEBUG --  agent 不 是 攻 击 者  "),
        (troop_is_hero,":attacker_trp_id"),
#         (display_message, "@{!}DEBUG --  攻 击 者 是 英 雄  "),
        (store_random_in_range,":random_int",0,100),
#         (display_message, "@{!}DEBUG --  取 0 到 100 的 随 机 数 "),
        (ge,":random_int",75),
#         (display_message, "@{!}DEBUG --  随 机 数 大 于  75 " ),
        (assign,":var_5",1),
        (assign,":var_6",2),
    (else_try),
        (eq,":agent_no",":attacker"),
#         (display_message, "@{!}DEBUG --  agent 是 攻 击 者  "),
        (try_begin),
            (key_is_down,56),
#             (display_message, "@{!}DEBUG --  key is down 56 "),
            (assign,":var_5",1),
            (assign,":var_6",2),
        (try_end),
    (try_end),
    ...
    (eq,":var_5",1),
#     (display_message, "@{!}DEBUG -- var_5 等 于 1 "),

    (assign,":missile","itm_light_arrows"),
#     (display_message, "@{!}DEBUG -- 子 弹 为 itm_light_arrows， 开 始 循 环 武 器 111"),
    (try_for_range,":var_9",0,4),
#     (display_message, "@{!}DEBUG -- 111 循 环 武 器 "),
        (agent_get_item_slot,":var_10",":attacker",":var_9"),
        (eq,":var_10",":attacker_weapon"),
#         (display_message, "@{!}DEBUG -- 所 取 slot 武 器 正 是 攻 击 武 器  "),
        (troop_get_inventory_slot_modifier,":var_11",":attacker_trp_id",":var_9"),
#         (display_message, "@{!}DEBUG -- 结 束 循 环 武 器 111  "),
    (try_end),
    (assign,":var_5",0),
    (try_for_range,":var_9",0,4),
#     (display_message, "@{!}DEBUG -- 222 循 环 武 器 "),
        (eq,":var_5",0),
#     (display_message, "@{!}DEBUG -- var_5 等 于 0 "),
        (agent_get_item_slot,":var_10",":attacker",":var_9"),
        (gt,":var_10",0),
#         (display_message, "@{!}DEBUG -- 所 取 武 器 正 常 " ),
        (item_get_type,":weapon_type",":var_10"),
        (assign,":var_13",0),
        (try_begin),
            (eq,":attacker_weapon_type",8),
#         (display_message, "@{!}DEBUG -- 攻 击武 器 类型为 8 " ),
            (eq,":weapon_type",5),
#         (display_message, "@{!}DEBUG -- 所 取 武 器 类型为 5 " ),

            (assign,":var_13",1),
        (else_try),
#         (display_message, "@{!}DEBUG -- 1113"),
            (eq,":attacker_weapon_type",9),
#         (display_message, "@{!}DEBUG -- 攻 击武 器 类型为 9 " ),
            (eq,":weapon_type",6),
#         (display_message, "@{!}DEBUG -- 所 取 武 器 类型为 6 " ),
            (assign,":var_13",1),
        (else_try),
#         (display_message, "@{!}DEBUG -- 1117"),
            (eq,":attacker_weapon_type",16),
#         (display_message, "@{!}DEBUG -- 攻 击武 器 类型为 16 " ),
            (eq,":weapon_type",182),
#         (display_message, "@{!}DEBUG -- 所 取 武 器 类型为 18 " ),
            (assign,":var_13",1),
        (else_try),
#         (display_message, "@{!}DEBUG -- 1118"),
            (eq,":attacker_weapon_type",16),
#         (display_message, "@{!}DEBUG -- 攻 击武 器 类型为 17 " ),
            (eq,":weapon_type",18),
#         (display_message, "@{!}DEBUG -- 所 取 武 器 类型为 18 " ),
            (assign,":var_13",1),
#         (display_message, "@{!}DEBUG -- 结 束 循 环 武 器 222"),

        (try_end),
        (eq,":var_13",1),
        (assign,":var_14",0),
        (assign,":var_15","itm_light_arrows"),
        (try_for_range,":var_16",0,4),
#         (display_message, "@{!}DEBUG -- 开 始 循 环 武 器 333"),
        ...
        (try_end),
        (try_begin),
#         (display_message, "@{!}DEBUG -- 1115"),
            (ge,":var_14",2),
            (val_sub,":var_14",2),
            (agent_set_ammo,":attacker",":var_10",":var_14"),
            (assign,":missile",":var_15"),
            (assign,":var_5",1),
        (try_end),
#         (display_message, "@{!}DEBUG -- 结 束 循 环 武 器 333"),
    (try_end),
    (eq,":var_5",1),
    ...
    (try_begin),
        ...
        (position_move_z,pos2,5),
        (add_missile,":attacker",2,":var_19",":attacker_weapon",":var_11",":missile",0),
#         (display_message,"@{!}DEBUG --  连 珠 发 射 成 功 "),
    (try_end),
#     (display_message, "@{!}DEBUG --  连 珠 触 发 器 结 束 "),
(try_end),
...
#连珠枪
["flintlock_pistol_con", "Flintlock Pistol222", [("flintlock_pistol",0)], itp_type_pistol |itp_merchandise|itp_primary ,itcf_shoot_pistol|itcf_reload_pistol, 230 , weight(1.5)|difficulty(0)|spd_rtng(38) | shoot_speed(20) | thrust_damage(45 ,pierce)|max_ammo(30)|accuracy(65),imodbits_none,
 [ (ti_on_weapon_attack,
   [(play_sound,"snd_pistol_shot"),
    (position_move_x, pos1,27),
    (position_move_y, pos1,36),
