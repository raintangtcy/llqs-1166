...
(try_for_agents,":agent"),
    ...
    (get_distance_between_positions_in_meters,":distance_between_attacker_agent",pos2,pos3),
    (agent_get_party_id,":agent_party_id",":agent"),
    (try_begin),
        (neg|gt,":distance_between_attacker_agent",2),
        (neg|eq,":attacker_party_id",":agent_party_id"),
        (agent_deliver_damage_to_agent,":attacker",":agent",10),
    (try_end),
(try_end),

(agent_get_team,":agentTeam"),
(agent_get_player_id,":player"),
(agent_get_team,":playerTeam", ":player"),
(try_for_agents),
(try_begin),
(eq, ":agentTeam", ":playerTeam"),
...
["nordic_guard_sword",         "Nordic Guard Sword", [("arabian_sword_c",0),("scab_arabian_sword_c", ixmesh_carry)], itp_type_one_handed_wpn|itp_bonus_against_shield|itp_unique|itp_primary, itc_longsword|itcf_carry_sword_left_hip|itcf_show_holster_when_drawn,
1500 , weight(3)|difficulty(9)|spd_rtng(95) | weapon_length(95)|swing_damage(36 , cut) | thrust_damage(28 ,  pierce),imodbits_sword_high ],
["nordic_rush_axe", "Nordic Rush Axe", [("one_handed_battle_axe_a",0)], itp_type_one_handed_wpn|itp_primary|itp_secondary|itp_bonus_against_shield|itp_unique|itp_wooden_parry, itc_scimitar|itcf_carry_axe_left_hip,
 1500 , weight(3.4)|difficulty(9)|spd_rtng(92) | weapon_length(80)|swing_damage(42 , cut) | thrust_damage(0 ,  pierce),imodbits_axe ],
["smith_slice",         "Smith's Slice", [("voulge",0)], itp_type_two_handed_wpn|itp_two_handed|itp_primary|itp_wooden_parry|itp_unique, itc_nodachi|itcf_carry_axe_back,
...
["jigong_melee",         "Halberd_Bow", [("jigong",0)], itp_unique|itp_type_polearm|itp_offset_lance|itp_two_handed|itp_primary|itp_penalty_with_shield|itp_bonus_against_shield|itp_unbalanced, itc_staff|itcf_carry_bow_back,
 90 , weight(5.5)|spd_rtng(92) | weapon_length(160)|swing_damage(40 , cut) | thrust_damage(30 ,  pierce),imodbits_none ],
 ...
 [
  ...
  (ti_on_weapon_attack,
   ...
   [(play_sound,"snd_pistol_shot"),
    ...
    (store_trigger_param_1,":attacker"),
(try_begin),
    (agent_get_troop_id,":attacker_trp_id",":attacker"),
    (agent_get_wielded_item,":attacker_weapon",":attacker"),
    (gt,":attacker_weapon",0),
    (item_get_type,":attacker_weapon_type",":attacker_weapon"),
    (is_between,":attacker_weapon_type",8,18),#武器类型在8-18之间，包含了枪，弓，弩
    (get_player_agent_no,":agent_no"),#获取agent ID
    (assign,":var_5",0),
    (assign,":var_6",0),
    (try_begin),
        (neg|eq,":agent_no",":attacker"),#如果agent不是攻击者
        (troop_is_hero,":attacker_trp_id"),#如果攻击者是英雄
        (store_random_in_range,":random_int",0,100),
        (ge,":random_int",75), #1/4的概率
        (assign,":var_5",1),
        (assign,":var_6",2),
    (else_try),
        (eq,":agent_no",":attacker"),#如果agent是攻击者
        (try_begin),
            (key_is_down,56),
            (assign,":var_5",1),
            (assign,":var_6",2),
    ...
    (assign,":missile","itm_cartridges222"),#指定发射物，可以为箭，驽矢，子弹
    (try_for_range,":slot_no",0,4),#0-4为人物身上前四个slot，对应四个武器位上的物品
        (agent_get_item_slot,":a_weapon",":attacker",":slot_no"),
        (eq,":a_weapon",":attacker_weapon"),#当前循环到的武器正是攻击所用的武器
        (troop_get_inventory_slot_modifier,":var_11",":attacker_trp_id",":slot_no"),
    (try_end),
    (assign,":var_5",0),
    (try_for_range,":slot_no",0,4),
        ...
        (agent_get_item_slot,":a_weapon",":attacker",":slot_no"),
        (gt,":a_weapon",0),
        (item_get_type,":weapon_type",":a_weapon"),
        (assign,":var_13",0),
        (try_begin),
            (eq,":attacker_weapon_type",8),
            (eq,":weapon_type",5),
            (assign,":var_13",1),
        (else_try),
            (eq,":attacker_weapon_type",9),
            (eq,":weapon_type",6),
            (assign,":var_13",1),
        (else_try),
        ...
        (eq,":var_13",1),
        (assign,":var_14",0),
        (assign,":var_15","itm_cartridges222"),
        (try_for_range,":var_16",0,4),
            (agent_get_item_slot,":var_17",":attacker",":var_16"),
            (eq,":var_17",":a_weapon"),
            (agent_get_ammo_for_slot,":var_18",":attacker",":var_16"),
            (val_add,":var_14",":var_18"),
            (assign,":var_15",":var_17"),
        (try_end),
        (try_begin),
            (ge,":var_14",2),
            (val_sub,":var_14",2),
            (agent_set_ammo,":attacker",":a_weapon",":var_14"),
            (assign,":missile",":var_15"),
            (assign,":var_5",1),
        (try_end),
    (try_end),
    (eq,":var_5",1),
    (str_store_troop_name,1,":attacker_trp_id"),
    (assign,":var_19",70),
    (assign,":var_20",16),
    (try_begin),
        (eq,":var_20",0),
    ...
    (store_proficiency_level,":var_21",":attacker_trp_id",3),
    (val_mul,":var_21",10),
    (val_add,":var_19",":var_21"),
    (try_begin),
        (eq,":var_6",2),
        (agent_get_look_position,pos3,":attacker"),
        (copy_position,pos2,pos1),
        (position_copy_rotation,2,3),
